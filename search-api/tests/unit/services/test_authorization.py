...
from tests.utilities.factory_utils import (
    factory_affiliation_model, factory_corporation_model, factory_membership_model, factory_org_model, factory_user_model)


def test_get_user_authorizations_for_corporation(session):  # pylint:disable=unused-argument
    """Assert that user authorizations for corporation is working."""
    user = factory_user_model()
    org = factory_org_model()
    membership = factory_membership_model(user.id, org.id)
    corporation = factory_corporation_model()
    factory_affiliation_model(corporation.id, org.id)

    authorization = Authorization.get_user_authorizations_for_corporation({
        'sub': str(user.keycloak_guid),
        'realm_access': {
            'roles': ['basic']
        }}, corporation.business_identifier)
    assert authorization is not None
    assert authorization.get('orgMembership', None) == membership.membership_type_code

    # Test with invalid user
    authorization = Authorization.get_user_authorizations_for_corporation({'sub': str(uuid.uuid4()), 'realm_access': {
        'roles': ['basic']
    }}, corporation.business_identifier)
    assert authorization is not None
    ...
    authorization = Authorization.get_user_authorizations_for_corporation(
        {'loginSource': 'PASSCODE', 'username': corporation.business_identifier, 'realm_access': {
            'roles': ['basic']
        }},
        corporation.business_identifier)
...
def test_get_user_authorizations_for_corporation_service_account(session):
    """Assert that user authorizations for corporation is working."""
    user = factory_user_model()
    org = factory_org_model()
...
def test_check_auth(session):  # pylint:disable=unused-argument
    ...
    check_auth({'realm_access': {'roles': ['public']}, 'sub': str(user.keycloak_guid)}, one_of_roles=OWNER,
               business_identifier=corporation.business_identifier)
